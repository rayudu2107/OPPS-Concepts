...
import com.training.book.entity.BookTransactionHistory;
import com.training.book.entity.User;
import com.training.book.exception.OperationNotPermittedException;
import com.training.book.file.FileStorageService;
import com.training.book.mapper.BookMapper;
import com.training.book.repository.BookRepository;
import com.training.book.repository.BookTransactionHistoryRepository;

import static com.training.book.book.BookSpecification.withOwnerId;
...
public class BookService {
    ...
    public Integer save(BookRequest request, Authentication connectedUser) {
        ...
        log.info("Book saved successfully with ID: {}", bookId);
    ...
    public PageResponse<BookResponse> findAllBooksByOwner(int page, int size, Authentication connectedUser) {
        log.info("Finding all books by owner");
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
    ...
    public Integer updateShareableStatus(Integer bookId, Authentication connectedUser) {
    	   log.info("Updating shareable status of book with ID: {}", bookId);
    ...
    public Integer updateArchivedStatus(Integer bookId, Authentication connectedUser) {
    	  log.info("Updating archived status of book with ID: {}", bookId);
        Book book = bookRepository.findById(bookId)
                .orElseThrow(() -> new EntityNotFoundException("No book found with ID:: " + bookId));
        User user = ((User) connectedUser.getPrincipal());
        if (!Objects.equals(book.getOwner().getId(), user.getId())) {
    ...
    public Integer returnBorrowedBook(Integer bookId, Authentication connectedUser) {
    	 log.info("Returning borrowed book with ID: {}", bookId);
    	Book book = bookRepository.findById(bookId)
                .orElseThrow(() -> new EntityNotFoundException("No book found with ID:: " + bookId));
        if (book.isArchived() || !book.isShareable()) {
    ...
    public Integer approveReturnBorrowedBook(Integer bookId, Authentication connectedUser) {
        log.info("Approving return of borrowed book with ID: {}", bookId);Book book = bookRepository.findById(bookId)
                .orElseThrow(() -> new EntityNotFoundException("No book found with ID:: " + bookId));
        if (book.isArchived() || !book.isShareable()) {
            throw new OperationNotPermittedException("The requested book is archived or not shareable");
        }
    ...
    public PageResponse<BorrowedBookResponse> findAllBorrowedBooks(int page, int size, Authentication connectedUser) {
    	  log.info("Finding all borrowed books");
    	User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
    ...
    public PageResponse<BorrowedBookResponse> findAllReturnedBooks(int page, int size, Authentication connectedUser) {
    	   log.info("Finding all returned books");
    	User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
